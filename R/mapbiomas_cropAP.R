...
infolder <- "C:\\Users\\Darren\\Documents\\2022_Norris_gdp_deforestaion\\AmazonConservation\\mapbiomas_ge\\state_cover\\AP"
...
# North municipalites
meuSIG <- file.choose()
#"C:\\Users\\Darren\\Documents\\TutoriaisPaisagem\\data\\vector_ibge\\munis_norte.shp"
# Mazagao Santana
meuSIG <- file.choose()
# "C:\\Users\\Darren\\Documents\\TutoriaisPaisagem\\data\\vector_ibge\\munis_maza_santana.shp"
mymunis <- st_read(meuSIG) 
mymunis <- st_union(mymunis) %>% st_transform(4326)
myexent <- ext(vect(mymunis)) 
#Cutias
meuSIG <- "G:\\vector\\AP_muni_Macapa_Cutias_clipbuff2km.shp"
mymunis <- st_read(meuSIG) 
mymunis <- st_union(mymunis) %>% st_transform(4326)
myexent <- ext(vect(mymunis))
...
rin <- "C:\\Users\\Darren\\Documents\\2022_Norris_gdp_deforestaion\\AmazonConservation\\mapbiomas_ge\\state_cover\\AP_utm_munis_maza_santana\\utm_cover_AP_munis_maza_santana_1985.tif"
rin <- "C:\\Users\\Darren\\Documents\\2022_Norris_gdp_deforestaion\\AmazonConservation\\mapbiomas_ge\\state_cover\\AP_utm_muni_cutias\\utm_cover_AP_muni_cutias_1985.tif"
plot(rast(rin))
#parallel example
library(doParallel)
## number of cores
